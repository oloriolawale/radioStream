...
.factory('User', ["$http", "$rootScope", "$location", "$localstorage", function($http, $rootScope, $location, $localstorage) {
  ...
  return {
    ...
    login: function(nonce, username, password) {
      return $http.jsonp( encodeURI( api+'user/generate_auth_cookie/?nonce='+nonce+'&username='+username+'&password='+encodeURIComponent(password)+'&insecure=cool&callback=JSON_CALLBACK' ) );      
    },
